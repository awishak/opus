...
async function loadDB() { try { const r = await window.storage.get(STORAGE_KEY); return r ? JSON.parse(r.value) : emptyDB(); } catch { return emptyDB(); } }
async function saveDB(db) { try { await window.storage.set(STORAGE_KEY, JSON.stringify(db)); } catch (e) { console.error(e); } }
...
function App() {
  ...
  const pick = useCallback(() => {
    ...
    if (matchMode === "least") {
      let pool = [...db.songs].sort((a, b) => (stats[a.id]?.m || 0) - (stats[b.id]?.m || 0));
    ...
    } else if (matchMode === "goliath") {
      const withStats = db.songs.map(s => ({ ...s, pct: wpn((stats[s.id]?.w || 0), (stats[s.id]?.l || 0)), m: stats[s.id]?.m || 0 }));
      const rankedPool = withStats.filter(s => s.m >= 10).sort((a, b) => b.pct - a.pct);
      const top25 = rankedPool.slice(0, Math.max(1, Math.ceil(rankedPool.length * 0.25)));
      const lowMatched = [...withStats].sort((a, b) => a.m - b.m).slice(0, Math.min(30, withStats.length));
      if (top25.length === 0 || lowMatched.length === 0) {
        // fallback to random if not enough ranked songs
        const sh = [...db.songs].sort(() => Math.random() - 0.5);
        setPair([sh[0], sh[1]]);
        return;
      }
      const goliath = top25[Math.floor(Math.random() * top25.length)];
      const davids = lowMatched.filter(s => s.id !== goliath.id);
      if (davids.length === 0) { const sh = [...db.songs].sort(() => Math.random() - 0.5); setPair([sh[0], sh[1]]); return; }
      const david = davids[Math.floor(Math.random() * davids.length)];
      // Randomize left/right position
      setPair(Math.random() < 0.5 ? [david, goliath] : [goliath, david]);
    } else {
      const sh = [...db.songs].sort(() => Math.random() - 0.5);
      let a = sh[0], b = sh.length > 1 ? sh[1] : db.songs.find((x) => x.id !== a.id);
      if (a && b && a.id !== b.id) setPair([a, b]);
    }
  }, [db.songs, matchMode, stats]);
  ...
  const enriched = useMemo(() => db.songs.map((s) => {
    ...
    return { ...s, w: st.w, l: st.l, m: st.m, pct: wpn(st.w, st.l) };
  }), [db.songs, stats]);

  const ranked = useMemo(() => enriched.filter((s) => s.m >= 10).sort((a, b) => b.pct !== a.pct ? b.pct - a.pct : b.m - a.m), [enriched]);

  const artists = useMemo(() => {
    const map = {};
    enriched.forEach((s) => { const k = s.artist.toLowerCase(); if (!map[k]) map[k] = { name: s.artist, songs: [] }; map[k].songs.push(s); });
    return Object.values(map).sort((a, b) => b.songs.length - a.songs.length);
  }, [enriched]);

  const decs = useMemo(() => {
    const map = {};
    enriched.forEach((s) => { const d = getDecade(s.year); if (!d) return; if (!map[d]) map[d] = []; map[d].push(s); });
  ...
  const Compare = () => {
    if (db.songs.length < 2) return (<div style={{ textAlign: "center", padding: "56px 24px", color: "#bbb" }}><p style={{ fontSize: 17, fontWeight: 600, color: "#aaa", margin: "0 0 6px" }}>Add at least 2 songs to start</p><button style={bp} onClick={() => setPg("add")}>Add Songs</button></div>);
      ...
      <div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
          <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
            <button style={tog(matchMode === "all")} onClick={() => { setMatchMode("all"); setPair(null); }}>All Songs</button>
            <button style={tog(matchMode === "least")} onClick={() => { setMatchMode("least"); setPair(null); }}>Least Matched</button>
            <button style={tog(matchMode === "goliath")} onClick={() => { setMatchMode("goliath"); setPair(null); }}>David vs Goliath</button>
          </div>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 36, marginTop: 24 }}>
          {[{ n: db.songs.length, l: "Songs" }, { n: db.matchups.length, l: "New Matchups" }, { n: ranked.length, l: "Ranked" }].map((x) => (
  ...
  const Standings = () => {
    const unranked = enriched.filter((s) => s.m < 10).sort((a, b) => a.m - b.m);
    const list = stFilt === "all" ? ranked : stFilt === "unranked" ? unranked : [...enriched].sort((a, b) => b.pct !== a.pct ? b.pct - a.pct : b.m - a.m);
    ...
    return (
      <div>
        <div style={{ display: "flex", gap: 5, marginBottom: 14, flexWrap: "wrap" }}>
          ...
          <button style={tog(stFilt === "everyone")} onClick={() => setStFilt("everyone")}>All ({db.songs.length})</button>
        </div>
        ...
        {filtered.length === 0 ? (<div style={{ textAlign: "center", padding: "40px 24px", color: "#bbb" }}><p style={{ fontSize: 14, fontWeight: 500 }}>{search ? "No matches" : "Songs need 10+ matchups"}</p></div>) : (
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead><tr><th style={{ ...TH, width: 34 }}>#</th><th style={TH}></th><th style={TH}>Song</th><th style={{ ...TH, textAlign: "right" }}>W</th><th style={{ ...TH, textAlign: "right" }}>L</th><th style={{ ...TH, textAlign: "right" }}>Pct</th><th style={{ ...TH, textAlign: "right" }}>GP</th></tr></thead>
    ...
    return (
      <div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 0 }}>
          {artists.map((a) => (<button key={a.name} style={{ display: "inline-flex", alignItems: "center", padding: "5px 14px", fontSize: 12, fontWeight: 500, background: "#f4f4f4", borderRadius: 100, cursor: "pointer", margin: 3, transition: "all 0.15s", fontFamily: FONT, border: "none", color: "#1a1a1a" }} onClick={() => setSelArtist(a.name)} onMouseEnter={(e) => { e.currentTarget.style.background = "#1a1a1a"; e.currentTarget.style.color = "#fff"; }} onMouseLeave={(e) => { e.currentTarget.style.background = "#f4f4f4"; e.currentTarget.style.color = "#1a1a1a"; }}>{a.name}<span style={{ display: "inline-block", padding: "1px 7px", fontSize: 10, fontWeight: 600, background: "#1a1a1a", color: "#fff", borderRadius: 100, marginLeft: 7 }}>{a.songs.length}</span></button>))}
        </div>
      </div>
    );
  };
  ...
  const Decades = () => {
    ...
    const noY = db.songs.filter((s) => !s.year).length;
    return (
      <div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
          {decs.map((d) => (<div key={d.d} style={{ background: "#fff", borderRadius: 14, padding: 22, border: "1px solid #ebebeb", cursor: "pointer", flex: "1 1 130px", textAlign: "center", minWidth: 110, transition: "all 0.15s" }} onClick={() => setSelDecade(d.d)} onMouseEnter={(e) => { e.currentTarget.style.borderColor = "#1a1a1a"; e.currentTarget.style.transform = "translateY(-2px)"; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = "#ebebeb"; e.currentTarget.style.transform = "translateY(0)"; }}><div style={{ fontSize: 22, fontWeight: 700 }}>{d.d}</div><div style={{ fontSize: 12, color: "#aaa", marginTop: 3 }}>{d.songs.length} songs</div></div>))}
        </div>
  ...
  const nav = [["compare", "Compare"], ["standings", "Standings"], ["add", "Add Songs"], ["artists", "Artists"], ["decades", "Decades"]];
  ...
  return (
    <div style={{ fontFamily: FONT, maxWidth: 720, margin: "0 auto", padding: "24px 16px", minHeight: "100vh", background: "#FAFAF9", color: "#1a1a1a" }}>
      ...
      <nav style={{ display: "flex", gap: 3, justifyContent: "center", margin: "18px 0 26px", flexWrap: "wrap" }}>
        {nav.map(([id, label]) => (<button key={id} style={pill(pg === id)} onClick={() => { setPg(id); setSearch(""); if (id === "compare") setPair(null); if (id === "artists") setSelArtist(null); if (id === "decades") setSelDecade(null); }}>{label}</button>))}
      </nav>
      {pg === "compare" && <Compare />}
